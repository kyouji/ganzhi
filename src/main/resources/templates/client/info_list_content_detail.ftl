   ...
   <div class="right_content newsBox">
    	...
    	<div class="crumb">
        	<a href="/">首页</a>
            <i>></i>
            <#if menu_id??&&menu_id != 10><a href="/info/list/${menu_id}">${menu_name}</a><i>></i></#if>
            <a <#if menu_id??&menu_id!=10> href="javascript:selectCat(${menu_id},${catId})" </#if>>${info_name.title!'' }</a>
        </div>
        <!--新闻列表-->
        <ul class="news_details">
        	<li>
            	<a href="#"><h3>${info.title!'' }</h3></a>
            	<#if menu_name == "最新资讯">
